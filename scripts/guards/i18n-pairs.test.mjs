...
test('findViolations', async (t) => {
  ...
  await t.test(
    'does not flag the zh half of a real t(zh, en) call, even with an embedded ' +
  ...
  await t.test(
    'does not span two unrelated JSX tags across plain statement code between ' +
  ...
  await t.test(
    'flags a zh half that ends in full-width punctuation directly against the ' +
  ...
  await t.test(
    'still does not flag a CJK label glued to a non-punctuation Latin/numeric run ' +
...
test('findViolations — (c) English-only JSX text/attribute', async (t) => {
  ...
  await t.test(
    'flags a user-facing attribute (title/aria-label/placeholder/label/description/' +
  ...
  await t.test(
    'does not flag an attribute whose value is a {t(...)} expression (never a plain ' +
  ...
  await t.test(
    'does not flag a JSX text run that looks like code (parens/braces/equals/dotted ' +
  ...
  await t.test(
    'does not flag a JSX text run that is lexically inside a t(...) call, however ' +
  ...
  await t.test(
    'skips *.test.ts(x) files entirely — assertions/fixtures are not rendered UI ' + 'copy',
    () => {
...
test('findViolations — (d) a CJK sentence glued to its own English translation', async (t) => {
  await t.test(
    'flags a string whose Latin tail (after the last CJK ideograph) is a 4+ word ' +
    ...
    () => {
      ...
      assert.deepEqual(findViolations(src), [
        {
          line: 1,
          text: '已达到单次读取上限 Reached the per-page limit, keep loading to see more',
        },
      ]);
    },
  );
  ...
  await t.test(
    'does not flag a short embedded technical term (< 4 words) — the established ' +
  ...
  await t.test(
    'does not flag a Chinese sentence whose trailing command tail (4+ words, but a ' +
  ...
  await t.test(
    'skips *.test.ts(x) files entirely — the string is wrapped in t(...) so only ' +
...
test('findViolations — (e) a glued zh/en pair inside a template literal', async (t) => {
  await t.test(
    'flags the short-pair shape with a `${}` interpolation in the middle, the task’s ' +
  ...
  await t.test(
    'flags a glued-sentence tail (4+ words) split across static segments by an ' + 'interpolation',
    () => {
      const src =
  ...
  await t.test(
    'does not flag a template literal whose only English is inside the ' +
  ...
  await t.test(
    'does not flag the zh half of a real t(zh, en) call built from template ' +
      'literals, even with a shared interpolation',
    () => {
      const src = 'const a = t(`已发布 ${n} 个 Operation`, `Published ${n} operations`);';
      assert.deepEqual(findViolations(src), []);
    },
  );

  await t.test(
    'correctly tracks a nested template literal inside an interpolation expression ' +
  ...
  await t.test(
    'does not corrupt outer-template tracking when a nested template literal’s own ' +
