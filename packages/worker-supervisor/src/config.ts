...
export interface SupervisorConfig {
   ...
   *  (`host-paths.ts` `hostModelsJsonPath`). Defaults to `${NEXTTIME_DATA}/models/models.json`
   *  (S7-A, docs/STATUS.md 维护者决定 2026-09-22 ⑤: moved out of config/ into its own directory —
   *  see packages/llm-proxy/src/config.ts's own doc comment on `modelsJsonOutFile`), the file
   *  `make gen-models` writes for the real provider. `MODELS_JSON_HOST_PATH` overrides it so an
   *  acceptance run can point spawned containers at a separately generated fake-provider file
   *  (deploy/accept/docker-compose.fake.yml, W6) without touching the production one. */
...
export function loadConfig(env: NodeJS.ProcessEnv = process.env): SupervisorConfig {
  ...
  return {
    ...
    nextTimeData,
    localDataDir,
    modelsJsonHostPath: env.MODELS_JSON_HOST_PATH || `${nextTimeData}/models/models.json`,
