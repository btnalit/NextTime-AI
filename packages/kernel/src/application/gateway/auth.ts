 ...
 */
export async function createOrReuseWebSession(
  client: PoolClient,
...
/**
 * S4.1 (design doc §7.11 "工作区上下文按请求给"): the console-session channel's equivalent of
 * {@link lookupPrincipalByApiKeyHash} — resolves the human Principal that is `userId`'s
 * membership in `workspaceId` (`principals.user_id`, migration 0019). Admin path for the same
 * reason as the API-key lookup (the workspace GUC cannot be trusted before the membership is
 * confirmed), pinned to one (workspace, user) pair by the `principals_workspace_user_key` unique
 * index. `null` when the user has no membership there, the Principal is disabled, or the
 * workspace itself is `disabled` (0019 `workspaces.status`) — all three read as "no access".
 */
export async function lookupMembershipPrincipal(
  pool: PoolLike,
...
/**
 * The console-session channel's counterpart of {@link authenticateHuman}: membership Principal
 * (above) + its create-or-reused `web` Session, so the rest of the gateway sees exactly the same
 * `AuthenticatedHuman` an API key would have produced. `null` = no membership.
 */
export async function authenticateUserInWorkspace(
  pool: PoolLike,
  ref: { readonly userId: string; readonly workspaceId: string },
): Promise<AuthenticatedHuman | null> {
  const principal = await lookupMembershipPrincipal(pool, ref);
  if (!principal) return null;
  const session = await withWorkspace(
    pool,
    { workspaceId: principal.workspaceId, principalId: principal.id },
