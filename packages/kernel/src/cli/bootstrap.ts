 ...
 *
 * Usage: node dist/cli/bootstrap.js create-workspace --name <ws> --owner <display-name>
 *
 * Creates the Workspace and its first Principal (`kind='human'`, `role='owner'` — design doc
 * §5.1.1), generates an API key, and prints it exactly once: only its sha256 hash
 * (`application/gateway/auth.ts`'s `hashApiKey` — the same hashing the human channel itself
 * uses to look the key back up, so the two can never drift) is stored, in
 * `principals.api_key_hash`. Reads `DATABASE_URL` from the environment via `createPool()`. `cli`
 * is a standalone entry point, like `index.ts`'s composition root — not part of the six-layer
 * dependency-cruiser rule (`.dependency-cruiser.cjs` matches only `substrate|governance|
 * application|adapters|interfaces`), so importing `application/gateway` here is unrestricted.
 */
