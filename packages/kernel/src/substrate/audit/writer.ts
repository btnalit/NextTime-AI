...
export interface AuditRecordInput {
   ...
   *  `actorPrincipalId` must be `null`. `actorUserId` is normally set too, but 遗留 54 (migration
   *  core 0032) also legalizes a platform row with no `actorUserId` — the operator CLI's
   *  unattributed purge (`application/platform/purge-workspace.ts`'s own doc comment has the
   *  detail; that caller always marks such a row `payload.attributedActor: false`). */
   ...
   *  遗留 54 unattributed-operator-CLI case above. */
