...
drop policy if exists ontology_versions_workspace_isolation on ontology_versions;
...
create or replace trigger ontology_versions_immutable_definition
...
drop policy if exists objects_workspace_isolation on objects;
...
drop policy if exists sources_visibility on sources;
...
drop policy if exists observations_visibility on observations;
...
create or replace trigger links_immutable_content
...
create or replace function links_block_delete() returns trigger
language plpgsql as $$
begin
...
create or replace trigger links_immutable_delete
...
drop policy if exists evidence_workspace_isolation on evidence;
...
drop policy if exists decisions_visibility on decisions;
