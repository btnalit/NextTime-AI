    ...
    <div className="stack" data-testid={testId}>
      ...
      <ol className="stack-s" data-testid="packaged-gate-steps">
        ...
        <li>
          ...
          <code className="mono">docker compose up -d {service}</code>。 Create the data directory
          and start the service.
        </li>
        <li>
          启动后它 announce 到内核，出现在平台「集成 → 门实例」；管理员<strong>启用</strong>
          它并把接入包 <code>{system}</code> 设为<strong>平台预置</strong>后，工作区 owner
          才能在「系统接入」启用。 After start it announces itself; an administrator enables it and
          presets its connector, then a workspace owner enables it on 系统接入.
        </li>
      </ol>
    </div>
  );
