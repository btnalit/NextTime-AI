...
首次部署装好即有管理员：按 `docs/runbooks/host-bootstrap.md`「首次登录」读出 kernel 生成的临时密码，
以登录名 `admin` 登录 web 控制台、改密后落在「概览」页；再用用户名 + 密码登录，在对话框里说出需求。
入口 agent 会在图上找 Worker 并拉起；需要审批的动作以卡片形式出现在「待我审批」，通过后执行，结果与
决策写回图。图的内容在 `/explorer` 查看。
