...
4. **确认按影响分级**（同一模式用于 ActionRequest 审批、工作区 / 用户清除、Handle 吊销、供应商密钥覆盖）：
   ...
   （S8 W1-A7 实现为三档：高 · 可逆并入中档——中档是锚定在触发器旁的确认，列出影响范围，强度不降；低档与不可逆档不变。）
...
- `ConfirmTier`（S8 W1-A7 起为 `components/kit/confirm`，tier = low / medium / irreversible）：分级确认的统一实现，不可逆档内含"键入名称 + 知情勾选"，
  取代 B2 提到的散落两步确认。
...
S6-B 的 `/providers/:id/secret` 是 501 stub。
...
**2026-09-22 维护者答复（上面 09-19 未答的一条）**：控制台写供应商密钥**不过审批**——管理面配置，不是 agent 动作；易用性优先，不加过度限制（只写不读、审计不记值，不另加重输密码）。要守的是 agent 永远拿不到 llm-admin token。S6-B 范围不变，写路径在 S7-A 实现（`STATUS.md` §3）。同日另定：`config/` 不改 owner，`models.json` 挪专用目录（遗留 50）；owner 不归档他人私有 Chat（遗留 45）；镜像发布与备份定时器排到最后。
