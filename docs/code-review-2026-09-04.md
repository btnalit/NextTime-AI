...
- **加固（2026-09-08 收口状态）**：agent-host / worker-supervisor 已改经 `docker-socket-proxy`（按实际调用面 allowlist：CONTAINERS/POST/NETWORKS/ALLOW_START/ALLOW_STOP）访问 Engine API，不再挂载 socket；`gatekeeper-docker` 仍直连 socket——待用第二个 proxy 实例（仅 CONTAINERS + ALLOW_RESTARTS）收口。backup 容器：root + `cap_drop: [ALL]` + 仅 `DAC_READ_SEARCH`（非 root 方案实测 `cap_add` 不进 effective set，见 `docs/runbooks/backup-restore.md`）。WorkerDefinition 级 egress deny 列表接线中（⑤b）；基于 Docker 事件的 egress 反注册仍未做。
