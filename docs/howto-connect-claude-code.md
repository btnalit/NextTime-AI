...
**控制台（推荐）**：登录后打开「我的账户」页（`#/me/account`，2026-09-25 控制台产品化重构方案 D3
挪到这里——之前在「访问」页），在「接 Claude Code / MCP」卡片里点「签发」——有效期、能力范围、可用的
门都在表单里选，签出的 Handle 只显示一次，卡片同时给出下一步要粘贴的 `claude mcp add` 命令（用你控
