...
| 接入向导第④步"提议重分类"提交后显示 `conflict`（409） | 该 Operation 是不是清单导入产生的（`origin:'import'`） | 见"已知缺口"第 11 条——内核规则，不是 UI bug；对已导入的 Operation 目前无法通过 propose/publish 改分类 |
...
11. **（S3.12）接入向导第④步"提议重分类"对刚导入的 Operation 结构性地总是 409**：`propose_operation` 的冲突守卫（`governance/gatekeepers/manifest.ts` 的 `isOwnProposalDraft`）只允许替换**同一提议者**此前通过 `propose_operation` 自己写的草稿——一条 `origin:'import'` 的 Operation（清单导入产生的，向导审核页上的每一行都是）从不满足这个条件，无论它当前是 draft 还是 published，都会被判定为冲突（`OperationIdentityConflictError`，HTTP 409 `conflict`）。控制台没有绕过或吞掉这个错误——按钮仍然调用真实的两个 capability，把内核的原话通过 `ErrorBanner` 显示出来；这是内核既有规则的结构性结果，不是本次 UI 改动的 bug，需要内核侧决定是否/如何为"清理刚导入清单里的分类"开一条新路径才能解决。
12. **（S3.12）目录页与接入向导都没有 Operation 调用/审批统计列**——`get_operation_stats` 未出现在 `packages/shared/src/capabilities.ts` 的注册表里；`CatalogPage.tsx` 的 Operations tab 留了一条 `TODO` 注释标注这个缺口，未编造数据。
