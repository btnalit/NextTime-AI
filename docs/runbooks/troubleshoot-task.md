...
| `worker_failed` | WorkerRun 状态变成 `failed`（容器非零退出）且重试已耗尽（`retry_count >= 1`） | `docker logs nexttime-task-<workerRunId>`（若容器已被清理，只能看 `worker_runs` 表本身，或等下次复现时更快捕获）；常见原因：pi 进程崩溃、`entrypoint.sh` 的 "worker-mode self-check" 未通过（`api_key_env`/`egress_no_direct_route` 两项之一，或 `egress_via_proxy` 的 `no_proxy_configured`；`egress_via_proxy result=warn reason=proxied_request_failed` 自 W8 起不致命，不会是退出原因。见 `docs/runbooks/host-accept-s2.md` step6 说明）、模型请求失败且无重试余地 |
