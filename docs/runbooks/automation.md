...
对应交付物：本仓库的 GitHub 自动化补全包——`.github/workflows/{codeql,scorecard,image-scan,release-please,auto-merge,pr-title}.yml`、
...
## 依赖更新怎么做（2026-09-25 决定）

不用 bot 做常规版本更新：`renovate.json` 已删除（它的 GitHub App 从未安装，配置从没生效过）；
...
故意的（major 或 pi 依赖）。没有标签但也没触发：检查 workflow 运行记录里 `dependabot` job 有没有跑
（`if: github.actor == ...` 条件没匹配上，通常是因为这个 PR 不是 Dependabot 开的）。
