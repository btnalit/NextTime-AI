...
- 实现说明（S2.13 PR，2026-09）：
  ...
  - **`find_operations`/`find-means.ts` 的一处真实 I16/I17 缺口，顺手修了**：`substrate/graph/find-means.ts` 自己的模块注释原来断言"published only 对三种元本体类型都是免费的，因为都是发布时才投影进图"——这句话对 `WorkerDefinition`成立（`projectWorkerDefinitionObject` 只在 `publishWorkerDefinition` 里调用），但 S2.4 落地的 `Operation` 走的是另一条路：`registerOperationDraftObject` 在**导入/提议时**就把 Object 写进 `objects`（`properties.status:'draft'`），`setOperationStatusObject` 之后只是原地把 `status` 改成 `'published'`——一份刚导入、还没发布的清单，`find_operations` 立刻就能搜到，直接违反 I16/I17（"未发布的清单对 agent 不可见"）与本任务自己的验收条目。修法：`findMetaOntologyObjects` 对 `Operation`/`Procedure`（`Procedure` 是 S2.14 会落地的同构未来类型，提前做成同一形状不额外成本）加一条 `properties ->> 'status' = 'published'` 过滤，`WorkerDefinition` 保持不加（它的 Object 属性里压根没有 `status` 字段，加了这条过滤会让它恒为空，是另一种错）。
  - **`find_operations` 与调用者 Grant 取交集，S2.7 留的"文档化空操作"接上了**：`application/task/service.ts` 的 `findOperations` 原来对 `caller` 参数完全不使用（S2.7 自己的注释："intersection 是文档化的 no-op，等 S2.4 落地真正的 Operation Object 与可核对的 Gatekeeper 关系"）。本任务把这条接上，形状照抄它自己的姐妹函数 `findWorkers`："`'unconstrained'`（owner、human 通道、没有 Handle 可收窄）看到全部已发布候选；一个真正的 `CapabilityScope` 只保留 `identityKey.gatekeeperId` 落在 `resources.gatekeeper` 里的候选"。**已知的、刻意的不对称**（PR body "已知偏离"重复一遍）：`request-action-handler.ts` 的 `observe` 路径（真正调 `<gate>.<op>` 工具那一步）本身完全不检查调用方 Handle 的 `resources.gatekeeper`——这是 S2.4 就存在的既有缺口，不在本任务所有权范围内，本任务不代为修；这里改的只是"`find_operations` 展示给 agent 的候选列表"这一层，不是唯一的强制点。
