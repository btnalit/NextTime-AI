...
| W2-U3b | #275 | 旅程①从 `test.fixme` 变为真跑：新建工作区与 owner（强制改密），全程走界面——空状态（就绪卡片两项缺失、三页前提条）→ 系统接入启用平台门（预览确认）→ 授权抽屉授权 → 缺项收窄为一项 → 能力目录从 ops-runner 模板创建、勾选该门、保存并发布 → 就绪卡片"已就绪"、三页前提条消失 → 对话收发 → 768 宽新建草稿 | CI 的 e2e 栈是 `AGENT_RUNTIME=fake`（只回显），没有 agent-host / worker-supervisor，任何消息都触发不了真实 `invoke_worker`：旅程①在 CI 里止于"三项前提齐备 + 对话可用"，委派本身由 `accept_s2`（主机）与维护者页面验收证明（遗留 83）。"无权限"状态与"委派前拦截"未覆盖（后者产品里尚无此机制） |
