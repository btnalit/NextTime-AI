...
- 实现说明（web console redesign PR，2026-09）：
  ...
  - **权限推断**：内核没有"当前 principal 角色"读取能力，`hooks/usePermissions.tsx` 记录本会话遇到的每个 403 capability，据此隐藏/替换 owner-/operator-only 的按钮（"Always allow this kind"、`list_pending` 徽标、Connections 的 owner 动作）。
