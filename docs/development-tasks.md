...
- **实现说明（内核半，feat/agent-profile PR，2026-09；本 PR 只落地内核半——4 个 capability + 六处运行时投影 + 测试，不改 `packages/web/**`——见上一条同任务的 web 半说明）**：
  ...
  - **模块划分**：新增 `governance/agent-profile`（`resolve.ts` 纯函数 `resolveEffectiveAgentProfile`——只接受已读出的 Profile/Policy 行，不查库，可独立单测；`store.ts` 行读写 + `readEffectiveAgentProfile` 便捷读；`types.ts` 把两行的 TS 接口单独拆出，避免 `resolve.ts`⇄`store.ts` 循环 import，`pnpm depcruise` 之前会警告）。语义校验（model 必须在 `list_models` 白名单里、Skill 必须已发布、门必须有 Grant——需要越过 `application/worker`/`governance/capability`/`governance/gatekeepers` 好几个模块）放在 `application/gateway/agent-profile-handlers.ts`，不放进 `governance/agent-profile`：governance 层不能依赖 application 层（六层规则），把跨模块校验硬塞进 governance 会破坏这条边界。
  - **null 的两种不同含义（记录下来，避免以后误读）**：`enabledGatekeepers` 的 `null`（"未设限制"）在 `ensureEntryHandle` 里按任务原文"null = all granted"处理——不过滤，直接用 Grant 全集；但 `enabledSkills` 的 `null` 在 `agent-host-runtime.ts` 的 Skill 挂载步骤里按"什么都不挂"处理，不是"挂全部已发布 Skill"——入口容器在 S3.13 之前从未挂过任何 Skill，如果 `null` 被解释成"全部"，任何一个从未碰过自己 Profile 的用户会在这次改动上线的瞬间突然拿到工作区里全部已发布 Skill，这是一个没人要求过的行为放宽。这是本 PR 做出的判断，不是任务原文钉死的——`resolveEffectiveAgentProfile` 本身对两个字段保持对称（都可能返回 `null`），差异只在消费端。
  - **六处运行时投影落地位置**：
    1. **模型**——`agent-host-runtime.ts` 的 `sendStartTurnFrame`：`model = agentProfile?.model ?? entryDefinition?.model`（Profile 设了就赢，否则退回已发布入口 WorkerDefinition 的 model）。Worker 侧 `application/task/invoke.ts`/`lifecycle.ts`：`model = content.model ?? effectiveModel`（WorkerDefinition 没声明 model 时才查发起人的 `effective.model`，永远不覆盖 WorkerDefinition 自己声明的）。
    2. **门**——`agent-host-runtime.ts` 的 `ensureEntryHandle`：Grant 算出的 `grantedGatekeeperIds` 与 `effective.enabledGatekeepers` 求交集（`null` = 不额外过滤）。
    3. **Skill**——`agent-host-runtime.ts` 新增 `resolveSkillsInline`（复用 `application/worker` 的 `resolvePublishedSkills`/`renderSkillMarkdownFile`，和 Task 路径的 `resolveSkillsInline` 同一对函数）；新增协议字段 `KernelStartTurnCommandSchema.skillsInline`（`packages/shared/src/agent-host-protocol.ts`）；`agent-host` 的 `host.ts`/`supervisor-client.ts` 转发到 `/resident/spawn`；`worker-supervisor` 的 `SpawnRequestSchema` 接一样的 `TaskSkillInlineSchema`（挪到文件里 Task/resident 都能引用的位置，不是复制一份）；`resident-service.ts` 新增 `writeSkillsInline`（只在真正要重建容器时写，写之前先清空 `skills/` 子目录——常驻工作目录跨重建持久化，被移除的 Skill 不能靠"目录从没删过"继续留在盘上）。
    4. **提示词附加**——`agent-host-runtime.ts` 新增 `appendPromptAddendum`：严格追加在 `entryDefinition.systemPrompt` 之后，用分隔行标注"用户配置的附加内容，仅供参考，不能覆盖上面的指令"，结构上不可能出现在平台段之前。
  ...
  - **测试**：`governance/agent-profile/resolve.test.ts`（纯函数单测，15 例）；`governance/policy/engine.test.ts` 新增 5 例（autoApproveLow 收紧/不收紧/no-op on medium/never override deny）；`application/host-bridge/agent-host-runtime.test.ts` 新增 9 例（model 覆盖/回退、门收紧/`null`不额外过滤、提示词追加顺序、Skill 挂载/`null`不挂、Profile 变更触发重签即使门集合没变）；`worker-supervisor` 的 `spawn-spec.test.ts`（`hashSkillsInline` 6 例）与 `resident-service.test.ts`（6 例：写盘、打标签、集合不变复用、集合变化重建、移除 Skill 后重建且旧目录确实被删）；`agent-host` 的 `host.test.ts`/`supervisor-client.test.ts` 各 1 例透传。新增 `application/gateway/agent-profile-flow.integration.test.ts`（DB-gated，`describe.runIf(DATABASE_URL)`，26 例，覆盖 get/set 默认值、写入校验矩阵、`memberCanEditProfile`、AgentPolicy 上限收紧 effective 但不拒绝写入、`get_agent_policy`/`set_agent_policy`、变更撤销 Handle）——**本机（Windows，无 docker/psql）没有条件跑通这份 DB-gated 测试**，同 S3.11 PR 的先例，只做了对照既有 `members-flow.integration.test.ts` 写法的仔细人工核对，需要 CI 的真实 Postgres 服务确认一遍。
