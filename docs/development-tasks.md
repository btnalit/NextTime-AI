...
- 实现说明（S2.7 PR，2026-09）：
  ...
  - **`create_task` 于 W5 下架（PR #130，2026-09-10）**：W4 曾按"只建不跑"接线，2026-09-10 复审（`code-review-2026-09-10.md` §3.2）确认它在唯一现实路径上不可达——它在 Worker ceiling 里、不在 entry ceiling 里，而 `computeChildHandleScope` 对父 Handle 没有的非 execute 能力静默丢弃，两处入口 Handle 签发都走固定的 `entryScope()`；同时它建出的 `queued` Task 计入每 principal 并发额度，reaper 只扫有 WorkerRun 的 Task，Worker ceiling 里又没有 `cancel_task`，额度只能由人回收。下架范围：注册表条目、`createTaskHandler`、`application/task/invoke.ts` 的 `createTask`、Worker ceiling 条目、集成测试块，契约快照同步；`insertQueuedTaskWithQuotaCheck` 保留为 `invokeWorkerCreate` 的单一调用者。接线留到授权衰减模型有结论之后。
