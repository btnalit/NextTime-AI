...
本批合入顺序：#296 → #297 → #299；三者都改截图，后合的先合 main 再重拍。经验：CI 的 e2e 各 spec 共用一个 Postgres，旅程测试留下的状态（停用的工作区）会破坏别的 spec 的空态断言——造了夹具就在测试末尾复原，与 `workspaces.spec.ts` 自己的"停用再启用"同一做法。
