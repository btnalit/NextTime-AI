...
- 实现说明（S2.4 PR，2026-09）：
  ...
  - **`request_action` 的判定表**（`application/gateway/request-action-handler.ts`）：① Operation 未发布（draft 或压根不存在）→ 视为 I17 未分类，`blast_radius=medium / auto_approvable=false / await_decision=true`，永远走 ActionRequest，永不直接执行；② `mode=observe` → 直接调门的 `observe`（开一个 Activity），`writeObservedFacts` 落图为 `observed` Fact，不产生 ActionRequest；③ `mode=execute` 解析成 `auto_approved` → 同一个 `client`/同一个事务内联执行（见下）并把结果直接返回；④ 解析成 `pending_approval` 且 `await_decision=true` → 轮询（仍是同一个 `client`）到超时或状态离开 `pending_approval`；轮到 `approved` 就内联执行，轮到 `rejected`/`expired` 原样返回，超时仍 `pending_approval` 就返回 `{status:'pending_approval', actionRequestId}`；⑤ `await_decision=false` → 立刻调门的 `simulate`，返回 `{status:'pending_approval', actionRequestId, simulate}`，不等待；⑥ 解析成 `denied` → 抛 `ActionRequestDeniedError`（继承 `ForbiddenError`，403）。
  - **为什么"内联执行"用同一个 `client`，不走 drainer 的独立连接模型**：`dispatchCapability`（`dispatch.ts`）把整个 handler 调用包在一个事务里，提交发生在 handler 返回**之后**——`requestAction()` 刚插入的 ActionRequest 行在 handler 返回前对任何其它连接都不可见（哪怕是同一个 `withWorkspace` 的新连接）。`ApprovalDrainer`/`ActionExecutor` 的设计（S2.3/本任务）刻意是"每次 DB 写开一个短事务、网络调用留在事务外"，套用到这里会导致刚插入的行谁也读不到。于是 `executeActionRequestInline`（`request-action-handler.ts`）直接复用 `governance/approval/execution.ts` 已经导出的 `startActionRequestExecution`/`markActionRequestExecuted`/`markActionRequestFailed`（它们本就不开自己的事务，接受调用方传入的 `client`），在同一个连接上把"开始执行 → 调门 apply → 写 observed Fact → 标记完成"跑完——**已知代价**：`await_decision=true` 且真等到超时（默认 90s，`REQUEST_ACTION_AWAIT_DECISION_TIMEOUT_MS` 可调）的调用会占住一个 DB 连接到那么久；这与设计本身"90 秒同步等待"的语义是一回事（更上层 `invoke_worker` 的 90 秒等待，S2.7，本质也是同一笔时间预算），代价被记录在这里而不是悄悄回避掉。
  - **内联执行与异步 drain 的竞态是预期内的，不是 bug**：`approve()` 之后既可能被本次 `request_action` 调用自己的轮询捡到（内联执行），也可能被 outbox consumer/定时 tick 触发的 drainer 捡到——两者都会调 `startActionRequestExecution`，行锁 + 条件 UPDATE（S2.3 的 `updateActionRequestStatusConditional`）保证只有一个赢，另一个拿到 `IllegalTransition`。`executeActionRequestInline` 遇到这个异常时**不会**重新调一次 `apply` 去找回执行结果（一次真失败后盲目重试可能对不保证幂等的下游系统造成二次执行；门自己的幂等存储按 `actionRequestId` 只保证同一个调用者重试安全，不是本函数的责任）——只回读行的终态（`executed`/`failed`/其它）如实上报，代价是这个稀有竞态下调用方拿不到 `data`（`status` 仍然正确）。`registerActionRequestDrainConsumer` 与组合根的定时 tick 反过来对称处理：`IllegalTransition` 静默吞掉（意味着"已被处理"），只把真正意外的错误交给 `onError`。
  - **组合根接线**（`packages/kernel/src/index.ts`）：`setRequestActionDeps`（只含 `gatekeeperClient`/`awaitDecisionTimeoutMs`）在 `createServer()` 里同步调用，不等 `createBackgroundServices` 的异步 `AgentRuntime` 引导——`request_action` 不依赖后者产出的任何东西。`ApprovalDrainer` 连同它的 `ActionExecutor`（`application/gateway/action-executor.ts`，组合 `adapters/gatekeeper-client` + `governance/gatekeepers` + `substrate`）、outbox consumer（`application/gateway/action-request-drain-consumer.ts`，订阅 `ActionRequestUpdated`，`status` 为 `approved`**或** `auto_approved` 都触发——后者是"没人同步等待的 `await_decision:false` 自动批准请求"兜底）、定时 tick（`GATEKEEPER_DRAIN_INTERVAL_MS`，默认 1 分钟，扫 `governance/approval` 新增的 `listDistinctExecutableGatekeepers`）都在 `createBackgroundServices` 里；三条触发路径都用同一个 `skipRoleSwitch:true` 的管理员事务（`SYSTEM_ACTOR_PLACEHOLDER` 是一个从不被当真实 Principal 解引用的占位 UUID，只满足 `withWorkspace` 的非空校验）——这类跨 workspace 后台任务与既有的 outbox dispatcher/审批 reaper 是同一类角色，不复用 per-request 的 RLS 事务。
  ...
  - **测试策略**：`packages/gatekeeper-base` 全部是纯单元测试（无网络，54 个用例覆盖四种传输 + 凭证 + 幂等存储 + 结果映射 + 协议 server 的 Fastify inject）；kernel 侧新增 `adapters/gatekeeper-client/index.test.ts`（单元，假 `fetch`）与三个 `describe.runIf(DATABASE_URL)` 集成测试（`governance/gatekeepers/{registry,manifest}.test.ts`、`application/gateway/request-action.integration.test.ts`——后者起一个真正的 `@nexttime/gatekeeper-base` Fastify 服务器监听本地端口，覆盖派发文字点名的五个验收场景：observe 写 Fact、auto-approve 执行一次、pending→approve→drainer 执行一次且重复 drain 不重复执行、未分类走 require_approval、draft 永不执行）；本地无 `DATABASE_URL` 自动跳过，经 CI 的 Postgres service 验证。
