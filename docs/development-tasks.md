...
- 实现说明（S3.2 PR，`feat/s3-2-conflicts-epistemic`，2026-09）：
  ...
  - **W5.5 实现说明（遗留 17，PR 待填）**：首次断言无行可锁、`links` 无身份唯一约束（Conflict 路径刻意保留两条活跃行），故在无既有 Fact 时按身份取事务级 advisory lock 并重读；仅首次路径加锁，稳态重复观测（已有活跃行）从不加锁。反向加锁（两批交叉锁不同顺序的一组首次身份）可能死锁，Postgres 会中止其中一方（`40P01` → 对该调用方呈现为 500），采集器的下个周期照常重提，写入本就幂等，批次因此是被重试而非丢失。
