...
- 实现说明（S3.2 PR，`feat/s3-2-conflicts-epistemic`，2026-09）：
  ...
  - **门（gates）**：`pnpm --filter @nexttime/kernel typecheck/lint`、`pnpm --filter @nexttime/shared typecheck/lint`、`pnpm -r typecheck`、`pnpm depcruise`（0 violations）、`pnpm ci:guards`（含 kernel-purity——`decisions.ts` 注释里原本直接写了 "Semantica"，被 kernel-purity 挡下后改成不点名，词表守卫另过）、`pnpm contract:snapshot` + `pnpm contract:check` 均过。本机无 Docker，DB-gated 套件本地自动 SKIP，验收结果见 `gh pr checks`。
