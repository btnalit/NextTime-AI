...
- 实现说明（S2.1 PR，2026-09）：
  ...
  - **`action_requests.policy_decision` 设为可空**，未按本条派发文字字面的 `not null`（已知偏离，DB 层判断胜过字面指示）：`packages/shared/src/transitions.ts` 的 `ACTION_REQUEST_EDGES` 有真实的 `proposed →(evaluate_policy)→ policy_evaluated` 转移，`proposed` 状态必须能在决策产生前入库——`not null` 会让这个状态完全无法插入，与 I6 冲突比它"保护"的东西更大。I7 改由两条 CHECK 实现：①（I6 支持）`status = 'proposed' or policy_decision is not null`；②（I7 核心）`status not in ('executing','executed','verified','compensated') or (policy_decision is not null and policy_decision <> 'deny' and (policy_decision <> 'require_approval' or approval_decision_id is not null))`。
