...
- 实现说明（S2.9 PR，2026-09）：
  - **Worker 写回的 Fact 是 `inferred`，靠 `CallerPrincipal.viaAgent`，不靠 `asserted_by` 的 principal 种类**（2026-09-08 回归修复）：review 修复 F8 让 `SqlGraphStore` 只从 `principals.kind` 派生 `epistemic_status`、忽略调用方自报的 `kind`（防止任何调用方自称 human 拿到 `asserted`）——但 `postWorkerResult` 的 `actorPrincipalId` 是 Task 的 `on_behalf_of` 人类（本代码库还没有 WorkerRun 自己的 agent 类 principal），派生结果把 agent 的推断记成了人类 `asserted`，`accept_s2.sh` 第 7 步失败。现在 `CallerPrincipal` 多一个 `viaAgent`：只能把状态压到 `inferred`、不能升格，`postWorkerResult` 写每条契约 Fact 时置 `true`（`substrate/graph/store.ts` `epistemicStatusForCaller`）。根治方向（待设计决定）：给 WorkerRun/WorkerDefinition 引入 agent 类 principal 作 `asserted_by`，`on_behalf_of` 留在 Activity/Session。
