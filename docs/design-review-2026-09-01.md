...
## 9. v0.3 增补：agent 必须能干活，系统必须能接进来（2026-09-01 补）

### 9.1 触发

你的两个追问：「假如啥都没有或者不行还能真的去干活吗」；「库存、报销、排班这些系统通过 CLI、MCP、API 拉起时，工具怎么调、接口怎么注入、怎么编排」。v0.2 的安全部分把 Worker 锁成了只能重启容器和查知识库，入口 agent 连文件都不能读；接入新系统要逐个写门。两条都不成立。

### 9.2 变化

| 项 | v0.2 | v0.3 | 依据 |
|----|------|------|------|
| 安全模型 | 无出网、内置工具白名单、入口 agent 子进程加每用户 OS 账号 | 三条底线：凭证不进 agent、碰有凭证 / 内网 / 有状态的系统才走门与审批、隔离与审计不减；其余放开 | 「凭证在哪，审批就在哪」 |
| agent 工作环境 | 只有平台工具 | 内置工具全开、装包、经出网代理上公网、Task 与用户工作目录 | 能干活的最小集合 |
| 入口 agent | agent-host 子进程 | 每用户一个常驻容器，与 Worker 同镜像；agent-host 只做事件桥 | 容器边界替代 I15 的目录规则，去掉 OS 账号 |
| 出网 | 禁止（I10） | 必经出网代理：公网放行、内网拒绝、域名入 Activity（I10 改写） | 抓网页、clone、pip 都需要 |
| 门 | 每系统一份代码；ssh / cli / mcp 在 P5 | 通用传输种类 `http` / `mcp` / `cli` / `ssh` + 接口清单（OpenAPI / MCP 导入、agent 提议、手写）+ 命令策略表；未分类默认要批（I17）；全部进 S2 | 接入任意系统不写代码 |
| 接口注入 | 未定义 | 扩展把 Handle 内的 Operation 注册为 `<gate>.<op>` 工具；Worker 只见工具名与参数 | 统一 http / mcp / cli / ssh |
| 连接新系统 | 未定义 | `request_connection` 卡片：人填地址与凭证，凭证直达门，图里生成门与系统对象 | cloudflare-os `requestConnection` |
| 编排 | 只有 `invoke_worker` | 三档：直接观察 / 委派任务 / 沉淀流程；审批按范围路由给持有者 | 库存查询不该拉 Worker，报销要财务批 |
| 沉淀 | Task / 会话 / 审计 | 加 Skill、Procedure、Operation 提议与发布；Worker 结果契约把持久知识写图 | 「下次别人能复用什么」 |
| 默认策略 | 全部要批 | `low` 自动批准，`medium` / `high` 与未分类要批 | 否则每步一次点击 |

### 9.3 三个场景的走法

设计 §8.6：运维（RouterOS 找流量大户并定位进程）、库存（OpenAPI 接入后直接观察）、报销（ConnectedAccount 以本人身份提交，财务按范围审批）、排班（第一次委派、沉淀为 Procedure、P5 由 Trigger 驱动）。四个场景同一套机制，只换本体模块与清单。

## 10. 工程质量复盘：分层、解耦、模块化、稳健性、维护（2026-09-01 补）

| 维度 | 发现 | 变化 | 落点 |
|------|------|------|------|
| 分层 | 内核十五个模块没有依赖方向，会长成 cloudflare-os 的 11k 行 Overseer | 六层单向依赖（domain → substrate → governance → application → adapters / interfaces），dependency-cruiser 在 CI 强制 | 设计 §7.10；R1、R3 |
| 解耦 | `llm` 代理在内核里，内核持有 provider key 且背着流式负载 | 拆为 `llm-proxy` 独立服务，本地验 Handle 签名；内核进程零外部凭证，I9 改写 | §7.7、§10.2；S1.7 |
| 解耦 | chat 直接消费 pi 的事件名 | `AgentRuntime` 适配接口，`host-bridge` 翻译为平台事件词表；pi 是唯一计划的实现 | §7.10；S1.4 |
| 解耦 | 审批路由、聊天推送、任务状态靠模块互调 | outbox 领域事件，同事务落库、重启重放；`chat` / `web` 只消费事件，不 import `approval` / `task` | §7.10；S1.4、S2.3 |
| 模块化 | 缺模块契约 | 每模块拥有自己的表与迁移，只经 `index.ts` 服务接口与事件被访问 | §7.10 |
| 模块化 | 「机制通用」没有物理形态 | 领域包（本体 + Skill + Procedure + Worker 定义）与接入包（清单 + 策略表 + 映射）是版本化 YAML；CI 规则：内核代码不得出现具体系统名 | §7.10、§10.1；R3 |
| 稳健性 | 没有失控防护 | I18 配额：派生深度、并发、token、时长、日成本；命中时的失败形态明确 | §5.4、§13；S2.7 |
| 稳健性 | 备份被暂缓 | 以 compose 内容器回到 S1，不改主机 | §10.2；S1.12 |
| 维护 | 缺规模门槛、测试分层、运行手册 | 单模块 ≤ 3000 行、单文件 ≤ 600 行；五层测试策略；七份运行手册 | §7.10；S3.10 |

刻意不加的：每门熔断器、ADR 目录拆分、capability 文档生成、第二种 agent 运行时。它们进 §17，不进切片。
