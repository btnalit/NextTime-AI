...
| `create_task` 接现有 spawn 路径，或先下架 | 内核 task | 完成（PR #130，下架）。注册表 / handler / `createTask` / Worker ceiling / 集成测试块一并移除，契约快照同步；理由见 `code-review-2026-09-10.md` §3.2 |
...
| 3 | `create_task` 的 Task 永远 `queued`；2026-09-10 复审确认它在唯一现实路径上不可达，**建议下架**（`code-review-2026-09-10.md` §3.2） | P2 | W5 | 关闭（PR #130：下架，接线留到授权衰减模型有结论之后） |
