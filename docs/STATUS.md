...
| 阶段评估与设计反思 | `retrospective-2026-09-09.md`、`retrospective-2026-09-18.md`、`retrospective-2026-09-19.md`（S6 无人值守实施）、`code-review-2026-09-04.md`、`code-review-2026-09-10.md` |
...
**当前波次：S6 控制台完善——本地实施完成，待维护者合入 / 发版 / 主机应用**（2026-09-19，无人值守实施；分支 `s6/console-completion` 基于 #216 的决定提交，**未推送、未开 PR**；实现说明见 `development-tasks.md` §5c，回顾见 `retrospective-2026-09-19.md`）。方式：主会话统筹，两波共十一条文件互斥车道（各自 worktree + 私有测试库），主会话审 diff、线性合入、补车道外接线；车道不写本文与 `development-tasks.md`。
...
| S6-A 页面 | 对话（归档 / 改名 / 自动标题 / 头部模式 · 模型 · 来源切换 / W3 修复 / 内联 ApprovalCard）、审批（reason + ConfirmTier + History 决定字段）、任务（关联审批 via taskId、取消确认、溯源链接）、审计（三种入口、ProvenanceChain、keyset 审计流、导出）、能力目录三个编辑器、工作区 / 用户（缺省过滤、残留预设、清除抽屉 irreversible、批量清理待激活用户）、成员 / 访问（RefChip、B7、双语）、概览残留横幅、C22 单测 | 完成（web 87+ 文件 / 559+ 用例；e2e 已写未执行） |
...
本地验证（2026-09-19，`s6/console-completion`）：`pnpm -r build / lint / typecheck` ✓、`depcruise` ✓、`ci:guards`（含新 `css-tokens`）✓、`contract:check` ✓；全包测试在全新测试库上——web 89 文件 / 570 用例、kernel 131 / 1400（全量并行时 outbox 两例超时、隔离 3/3）、llm-proxy 14 / 119、其余全绿；**Playwright e2e 全套 36 / 36** 在本机按 `e2e.yml` 步骤搭的 compose 栈上通过（一处测试过度断言已修），`accept_s1.sh --lite` 13 PASS + 6 SKIP；栈上抽查 CSP / `/api/llm-admin` 守卫 / Explorer 探测 / 镜像内 `KERNEL_VERSION`。细节见 `retrospective-2026-09-19.md` §2。**未做**：推送 / PR / 合入 / 发版 / 主机应用（S6 主机应用注意见 `development-tasks.md` §5c 末）。

**待维护者决定（阻塞项）**：① S6-B 控制台写供应商密钥是否属于"触及有凭证系统的动作必经审批"（`console-completion-plan.md` §12 末；P-B2a 页面直接录入门凭证是既有先例，但本次未据此推断）；② 镜像发布（S5.8 第 4 项重评）；③ 遗留 6（E7 备份定时器）；④ owner 是否应能归档他人**私有** Chat（遗留 45）；⑤ `${NEXTTIME_DATA}/config/` owner 改 10001 的取舍（S6-B，`host-llm-proxy-init.sh`）。
...
| 41 | `accept_s3.sh` 每次运行都把新铸的采集器 Handle 写进生产的 `secrets/collector-host-inventory.token`（脚本头注释里承认的"共享状态取舍"）。主机实测后果：生产采集器自 09-11 最后一次 S3 验收起一直 401（token 指向已禁用的 accept-s3 工作区），生产工作区一周没有采集数据、没人发现。升级 v0.13.0 时重铸 Handle 并发布 ops-assets 才恢复。拟修：改成 `demo.sh` 的做法——铸到私有文件、`docker compose run -e NEXTTIME_HANDLE_TOKEN_FILE -v …:ro` 只对本次 `--once` 生效、结束即删；另加一条不变量或告警：采集器连续 N 轮 401 / 非零错误要在 `/internal/metrics` 可见 | P2 | S6 | 关闭（本地分支，待合入：`accept_s3.sh` 改为 `demo.sh` 做法——Handle 铸到 `${NEXTTIME_DATA}/accept/` 私有文件、`docker compose run -e NEXTTIME_HANDLE_TOKEN_FILE -v …:ro` 只对本次 `--once` 生效、EXIT / HUP trap 删除，生产 `secrets/collector-host-inventory.token` 不再被写；采集器 interval 模式每轮失败日志带 `consecutiveFailures` / `kernelStatus`，连续 `HOST_INVENTORY_FAILURE_STREAK_ALERT`（3）轮转为 error "collector failing repeatedly"；内核新增 `ops.collector_silent`（active + standard 工作区里 service Principal 的 Source 超 2h 无观察）进 `/internal/metrics`） |

| 42 | 扩展 `modes/worker.ts`：`report_result` 工具立刻回复 "Result contract recorded."，真正的 `report_task_result` 在 `agent_settled` 才 POST；内核拒绝（400 / 403）时只记日志、以 0 退出（避免触发 S2.7 重跑），模型永远看不到拒绝、无法改正。拟修：`report_result` 时同步上报并把内核错误作为 `isError` 回给模型（保留 `agent_settled` 兜底），或至少把 `factsRejected` 回显给模型；I16 的 403（元本体类型）仍整单拒绝，同样需要回显 | P2 | S6 | 关闭（本地分支，待合入：`report_result` 同步上报，内核拒绝以 `isError` 回给模型（可修的 400 最多两次，403 / 409 直接结束本轮并带答复），`factsRejected` 等单条拒绝经 `get_task` 读回回显；`agent_settled` 仍无条件 exit 0；真 pi SDK 测试证明模型看得到并能改正、且不会空转；注：#211 后 I16 已是单条拒绝而非 403） |
| 43 | `await_decision: false` 的门工具下，真实模型 Worker 在请求动作后立刻 inspect、看到"还没执行"就 `report_result`，摘要写"未执行、待审批"——两秒后动作才落地；`tasks.result.summary` 与事实不符（#208 之后 Task 已能完成，但摘要仍是过时的）。属 Worker prompt 契约：Worker 应在 `report_result` 前对未决 ActionRequest 轮询 `get_action` 或明确交代"结果以 ActionRequest 状态为准"；进 S5.4 的 prompt-contract 守卫范围 | P3 | S6 | 关闭（本地分支，待合入：Worker 无法轮询（`get_action` 为 human-only），prompt 改为"引用 actionRequestId、结果以 ActionRequest 状态为准、不写未执行 / 失败"，门工具 `pending_approval` 结果同句提示；守卫允许名单加 `actionRequestId`、拒绝 `get_action` 有单测；需发布新 ops-runner 版本、主机真实模型复跑待做） |
| 44 | 常驻入口容器重建与进行中 Turn 竞争：`connect_gatekeeper` 等改变工作区门集合 / egress 后，下一轮 Turn 开始时内核 `POST /resident/spawn`，supervisor（#187 reconcile）发现规格已变而 `docker stop` 旧容器重建——这一轮 Turn 已派给旧容器，agent-host 报 `interrupted`。主机首次 `make demo` 在 Q3 撞上（#210 把 demo 的门连接挪到首轮对话前规避）；真实用户在对话中途接入门也会撞上。拟修：spawn 路径里"需要重建"时先完成重建再派发 Turn（或把重建推迟到当前 Turn 结算后），并加集成测试 | P2 | S6 | 关闭（本地分支，待合入：根因是 agent-host 把重建中旧容器的流关闭错记到尚未派发的新 Turn，非 supervisor 次序（`spawn()` 本就先重建后返回、`reconcile()` 从不停容器）；Turn 绑定容器 id，非绑定容器的事件一律忽略；三例确定性测试，修复前失败；中途接门推迟到下一轮 Turn，进行中 Turn 不受影响；主机对话中途接门复跑待做） |

| 45 | owner 归档他人 Chat 只达 RLS 已可见的 Chat（`visibility='workspace'`，今天没有能力会设置它）；他人**私有** Chat 对 owner 是 404（`chats_visibility`，"存在性不泄露"）。要覆盖需 security-definer 更新函数（0013 / 0021 先例）——隔离放宽，未自行做 | P3 | 决定 | 待维护者决定 |
| 46 | agent-host `sweepIdle` 只按 Turn 开始时的 `touch` 计时：超过 `entryIdleTimeoutMs`（30 min）的进行中 Turn 仍可能被中途停掉（S6 修 44 时发现的既有暴露） | P3 | S7 候选 | 开放 |
| 47 | `report_result` 在 Task `waiting_approval`（`await_decision: true` 超时）时仍 409 → 本轮结束 → 兜底重发 409 → exit 0 → 审批通过后 `no_result`（S6 修 42 时看到的既有路径） | P3 | S7 候选 | 开放 |
| 48 | 图谱页依赖的内核缺口：`traverse` 线上无 `direction` 且只回 id（页面改用 `state_at`）；无批量 Object 读（邻居名称 N × `get_object`）；`list_conflicts` 无对象 / Fact 筛选；观察窗口无能力暴露（页面写死 2h）；`GateInstanceWire` 只有 `enabledWorkspaceCount` 无工作区列表；`list_skills` 无 `markdown` / 无 `get_skill`（Skill "编辑"无法预填）；`propose_skill` / `propose_procedure` 无家族 id（"编辑"= 复制成新家族）；`list_grants` / `list_principals` / `list_skills` / `list_procedures` / `list_worker_definitions` / `list_tasks` 无分页 | P3 | S7 候选 | 开放 |
| 49 | 前端体积：@fontsource 附带 201 个未用 `.woff`、CSS 因 204 条 `@font-face` 涨到 275 KB（`dist` 13 MB）；`style-src` 仍需 `'unsafe-inline'`（83 处 `style={{…}}`）；主 JS chunk > 500 KB 的 Vite 警告（S6 前已有） | P3 | S6 收尾 | 开放 |
| 50 | S6-B 的两处判断：`ProviderStore` 并发写无互斥（单管理员前提）；`${NEXTTIME_DATA}/config/` owner 改 10001 才能让 llm-proxy 原子重写 `models.json`（`host-llm-proxy-init.sh`）——若维护者不接受，改为 supervisor / kernel 读另一路径 | P3 | 决定 | 待维护者决定 |
