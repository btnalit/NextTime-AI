...
| 9 | 领域包烤进 kernel 镜像（§5.7）；采集器 Source 状态按文件缓存（§5.9） | P3 | S5.3（W10-A） | 关闭（PR #195：`seed-domain-pack` 缺省从主机 `config/ontology/`（kernel `DOMAIN_PACK_DIR`，经既有只读 `config/` 挂载）读，放文件即 seed、不重建 kernel；`register_source` 按 (kind, name) 幂等（迁移 core 0028 `sources.name` + 唯一索引），采集器每轮注册、无本地状态） |
