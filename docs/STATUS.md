...
| 21 | 控制台看不到审批历史：注册表只有 `list_pending` 与 `get_action`，无列出已决 ActionRequest 的能力（`code-review-2026-09-10.md` §3.4） | P2 | S5.5（W10-B） | 关闭（PR #TBD：新增能力 `list_action_requests`（`status`/`gatekeeperId` 过滤、keyset 游标分页，同 `list_pending` 的 I14 可见性），`governance/approval/reads.ts` `listActionRequestsForApprover`；控制台 Approvals 页（`/work/approvals`）加"History"标签页，替换原会话级"All"标签的占位说明，详见 `development-tasks.md` S5.5 实现说明） |
