...
| 25 | CI 偶发：`interfaces/ws/server.test.ts` 的 WS 端到端用例在 PR #140 首跑时 5 秒超时，重跑通过（其余 1100 用例均过）；疑为 runner 争用，若复现需给该用例单独 `testTimeout` 或查 listener 启动时序 | P3 | S5.6（W10-C） | 关闭（PR #200：该用例单独给 `testTimeout: 15000`，vitest 默认 5s 与其自身 `waitUntil` 的 5s 内部超时几乎无余量；未改用例语义） |
