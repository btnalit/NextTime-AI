...
- **真实模型的路径现在存在了**（`--real <provider/model> [--runs N]`，PR #156）。W7 主机上用
  ...
  |---|---|---|---|
  | docker_restart | 0/10（动作已执行、Task `no_result`：S5.1 `reject` 拒绝 Worker 附带的 Fact 断言、整份结果契约被拒——遗留 30 第二根因） | 7/10（3 次失败全是入口 agent 没派工 `task=none`，模型行为；7 次派工的全部完成，修复确认） | 待填 |
  | api_observe | 10/10 | 10/10 | — |
  | ssh_run_approve | 3/10（7 次同一根因） | 5/10（5 次失败 = handler 预校验整单拒绝：Fact 引用元本体类型 I16 ×2、`proposedOperations` 的 `gatekeeperId` 是模型编的 ×3——#211） | 待填 |
  | ssh_run_auto | 1/1 | 0/1（同上 ×1） | 待填 |
...
**下一波**：按原顺序 P-B2b → P-C → P-D；S6 候选：遗留 41（`accept_s3.sh` 覆盖生产采集器 token + 采集器错误率进指标）、42（扩展回显内核拒绝）、43（Worker 结果 prompt 契约）、44（常驻容器重建与 Turn 竞争）、镜像发布。
